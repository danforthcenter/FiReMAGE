#FiReMAGE (Orthofinder version) local example set up

#paths & files names can be changed in 0. Setting up
...
#set working directory to the example_test folder
setwd("./example_test/")
...
metaTable <-
  fread(file = "./data/metaTables/V1_3s_metaTable.csv",
        sep = ",",
...
orthologs <-
  fread(
    file = "./data/OrthoFinder_orthologs/Orthogroups_tutorial_subset.csv",
    header = T,
    sep = ",",
    stringsAsFactors = F
  )

# permutations, 10 is usually good for a quick test
...
## for the tutorial 95% and 99% will be almost the same size we are using a low numPermutations and small subset of data
