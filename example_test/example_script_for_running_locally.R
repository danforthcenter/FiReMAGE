#Running the example data with FiReMAGE (Orthofinder version) for RStudio
...
orthologs <-
  fread(
    file = "./example_test/data/OrthoFinder_orthologs/Orthogroups_tutorial_subset.csv",
    header = T,
    sep = ",",
    stringsAsFactors = F
  )
...
output <- "./example_test/example_output/"
...
## NA warningss are from all_gene_coords is trying to change mitchondria (M) and chloroplast (C) chr to integers
